//$(document).ready(() : Memastikan bahwa load data dieksekusi setelah tabel html sudah tergambar di browser
$(document).ready(() => {
    loadData();

    //(e) Event Listener => menunggu kejadian yang terjadi
    ...
    $('table tbody').on('click', '.btn-delete', function () {
        if (confirm('Yakinkah data anda akan dihapus?')) {
            let id = $(this).attr('dataid');
            deleteData(id);
        }
    });
...
//Edit (Method GET), Show data in edit-modal ==============
const dataModal = id => {
    $.ajax({
        method: 'GET',
        url: `http://localhost:3000/api/` + id,
